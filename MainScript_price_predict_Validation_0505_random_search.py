...
from ABM_function import KGE_stat, create_dir, generating_ABM_samples
# import scipy.stats
...
#  your ROI (return on investment) is 18.2% per year. In some other states, the payoff period can be 
...
# read demand and price data for generating demand curves
...
# Cost 
dict = {'NG': [Cost_NG],'Solar':[Cost_s],'Wind':[Cost_w]}  # costs in dictionary
cost_t = [Cost_NG, Cost_w, Cost_s]      # costs at time t
file = os.path.join(Price_dir,'price_data_agg.csv' )  # file name ofprice data
price_monthly = pd.read_csv(file)  # monthly price
df_cost = G_hist_cost              # historical price data
...
IRR_threshold = 0.08  # IRR threshold 
...
# hist, bin_edges = np.histogram(agt_size_dist, density=True) # counts, bin_edges: bins
...
renewable_rec_dist = list(np.random.normal(1,0.2,renewable))  # generate random samples
# reneable companies preceive different incentive 
rec_dist =  renewable_rec_dist + list(np.zeros([1,n_agt-renewable])[0])  # non-reneable companies received no REC.
...
agt_risk_f = np.random.normal(0.8,0.2,n_agt)  # agents are risk-adverse; agents' risk attitude are assumed 
...
for k in range(10):  # explore the incentive amount that can reproduce historical data
    ...
    for t in range(9): # only zonal demand data only available from 2011 to 2020
       ...
       # demand_increase = 0.05  # annual demand increase rate
        ...
        supply_curves = hist_price(years)   # the supply curves [x_coeff, interception] of the load zone markets
        ...
        for agt_i in range(n_agt):
            ...
            new_cost_dict = {"NG":new_cost["NG"].values[0]*ng_cost_dist, "Wind":new_cost["Wind"].values[0]*wind_cost_dist[agt_i], 
...
df_KGE_file = os.path.join(cwd,"00 Results","Calibration", "KGE_" + date_now + ".xlsx")
