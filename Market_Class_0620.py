...
class Market(object):
    ...
    def __init__(self, file):
        self.file = file
        self.data_dir = 'c:/School/SloanProject/ABM_Model/Data'   # data directory path; a class attribute
...
def Evaluation(p, cf, cost, G_ls, G_tech): 
    ...
    if G_tech == 'NG':
        ...
        cash_flow = list(G.capacity*(p - variable_cost)*8760*G.cf*0.01*np.ones(G.life_span)) # has fuel and O&M costs
        cash_flow.insert(0,-x_invest) 
    else:      
        cash_flow = list(G.capacity*p*8760*G.cf*0.01*np.ones(G.life_span)) # no fuel cost and O&M cost
        cash_flow.insert(0,-x_invest)
